<div align="center">

# Hola, soy Valentín Pérez 👋

### Técnico Universitario en Programación · Desarrollo de software · Sistemas · Análisis funcional

[![LinkedIn](https://img.shields.io/badge/LinkedIn-Valentín_Pérez-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/valentin-ivan-perez)

</div>

## Sobre mí

Soy Técnico Universitario en Programación, graduado de la UTN Haedo. Tengo experiencia práctica en proyectos web y sistemas de gestión, participando en el relevamiento de requerimientos, la definición de funcionalidades, el trabajo con datos, la implementación, las pruebas funcionales y la resolución de problemas.

Me interesa seguir creciendo en equipos donde pueda combinar tecnología, análisis, organización y trabajo colaborativo.

- 📍 Buenos Aires, Argentina
- 🎓 Técnico Universitario en Programación — UTN Haedo
- 🔎 Intereses: desarrollo de software, análisis funcional, soporte de aplicaciones, datos y gestión de proyectos tecnológicos
- 🤝 Experiencia en proyectos académicos, freelance y colaborativos

## Proyectos destacados

| Proyecto | Tipo | Mi participación | Estado |
| --- | --- | --- | --- |
| [Sitio web para Inmobiliaria Martínez](https://github.com/Valeen-P/inmobiliaria-martinez-case-study) | Proyecto freelance para un cliente real | Relevamiento, estructura, desarrollo, validaciones y adaptación responsive | En uso |
| [Sistema académico y de certificación — UTN](https://github.com/Valeen-P/sistema-validacion-utn-case-study) | Proyecto académico colaborativo | Análisis, diseño funcional, gestión de información, implementación y pruebas | Finalizado |
| [Plataforma de turnos para profesionales](https://github.com/Valeen-P/psicologos-app-case-study) | Proyecto colaborativo con Ian Acevedo | Desarrollo integral compartido: requerimientos, flujos, implementación, revisión funcional y resolución de errores | En desarrollo |
| [Portfolio profesional](https://github.com/Valeen-P/portfolio-profesional) | Proyecto personal | Diseño y desarrollo de mi presentación profesional | En evolución |

> Algunos proyectos mantienen su código fuente en privado por confidencialidad, propiedad compartida o porque pertenecen a clientes. Los casos de estudio públicos muestran objetivos, decisiones y resultados sin exponer información sensible.

## Conocimientos

### Desarrollo y datos

- HTML, CSS y JavaScript
- PHP y MySQL
- C#
- SQL y PL/SQL
- Python básico
- Bases de datos relacionales
- Git y GitHub

### Análisis y calidad

- Relevamiento y análisis de requerimientos
- Validación de flujos y funcionalidades
- Pruebas funcionales
- Documentación funcional
- Resolución de problemas
- Atención al detalle y trabajo en equipo

## Actualmente

- Mejorando y documentando mis proyectos para presentarlos profesionalmente.
- Profundizando mis conocimientos de desarrollo, sistemas y análisis funcional.
- Participando en la construcción completa de una plataforma de gestión de turnos para profesionales.

## Contacto

Podés conocer más sobre mi experiencia o contactarme desde [LinkedIn](https://www.linkedin.com/in/valentin-ivan-perez).

---

<div align="center">

Gracias por visitar mi perfil.

</div>
